...
const VerifyOtp = () => {
  ...
  const handleVerify = async()=>{
    try {
      const res = await publicRequest.post("/auth/verify/new-password", {phone_number, otp})
    if(res.data.msg == "otp is approved"){
      //success
      Swal.fire(
        'Good job!',
        'You are now verified to create new password!',
        'success'
      )
      navigate(`/new-password/${phone_number}`)
    }else{
